...
export const convertDataForAgeAndCountStatistic = (data: IPeopleResult[]) => {
    ...
    const groupedByAge = updatedArray.reduce((acc: {ageGroup: string, male: number, female: number}[], currentItem) => {
    ...
    return groupedByAge.sort((a, b) => {
        return groupedByAgePropertyOrder.indexOf(a.ageGroup) - groupedByAgePropertyOrder.indexOf(b.ageGroup);
    });
} 